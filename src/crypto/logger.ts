...
class Logger {
  ...
  private saveLog(
    ...
    storage.get(logKey).then(async (existingLogs) => {
      ...
      await storage.set(logKey, updatedLogs)
    })
  }
  ...
  async serializeLogs(): Promise<string> {
    ...
    const logs: StoredLogData = {
      debug: await storage.get<string>("logs-debug") || "",
      info: await storage.get<string>("logs-info") || "",
      warn: await storage.get<string>("logs-warn") || "",
      error: await storage.get<string>("logs-error") || "",
    }
