  ...
  chains: [
    {
      ...
      blockExplorerUrl: "https://quaiscan.io", //https://quaiscan.io/
    }
  ],
} as Network
...
export function providerForNetwork(network: EVMNetwork): SerialFallbackProvider | undefined {
  const supportedNetworks = [QUAI_NETWORK];
  const providers = {
    evm: Object.fromEntries(
      supportedNetworks.map((network) => [
        ...
        makeSerialFallbackProvider(
          network.chainID,
          CHAIN_ID_TO_RPC_URLS[network.chainID] ?? []
        ),
      ])
    ),
  }
  setProviderForShard(providers.evm[network.chainID]);
  return providers.evm[network.chainID]
}
...
export function setProviderForShard(
  ...
  for (const provider of providers.providerCreators) {
    if (provider.shard !== undefined && provider.shard == shard) {
      ...
      console.error("Provider is not a Quais provider")
    } else if (
  ...
  console.error(`No provider found for shard: ${shard}`)
  return providers
}
...
export function ShardFromRpcUrl(url: string): string {
  ...
  console.error(`Unknown shard for rpc url: ${url}`)
  return ""
}
...
export function ShardToMulticall(shard: string, network: EVMNetwork): string {
  ...
  console.error(`Unknown mutlicall for shard: ${shard}`)
  return ""
}
...
export function CurrentShardToExplorer(
  ...
  console.error(`Unknown explorer for shard: ${currentShard}`)
  return ""
}
